...
class ProviderRegisterRequest(BaseModel):
    ...
    application_name: str = Field(..., min_length=3, example="Report Builder")
...
class SortByEnum(enum.Enum):
    """
    Defines sortable fields for provider queries.

    Attributes:
        name (str): Sort by provider name.
        status (str): Sort by provider status.
...
class OrderEnum(enum.Enum):
    """
    Defines the sorting order for provider queries.
...
class ProviderDetailsResponse(BaseModel):
    """
    Represents the response model for provider details.

    Attributes:
        id (UUID4): Unique identifier of the provider.
        name (str): Name of the provider.
        status (StatusEnum): Current status of the provider (active, inactive, revoked).
        user_id (UUID4): Unique identifier of the associated user.
        comment (str): Additional comments or notes related to the provider.
...
class ProviderUpdateRequest(BaseModel):
    """
    Represents the request model for updating provider details.

    Attributes:
        name (Optional[str]): The updated name of the provider.
        status (Optional[str]): The updated status of the provider. Expected values: "active", "inactive", "revoked"
        comment (Optional[str]): comments or notes regarding the provider to update.
...
class PaginatedResponse(BaseModel):
    ...
    Attributes:
        ...
        items (list[ProviderDetailsResponse]): The list of items retrieved for the current page.
    """
    ...
    page_size: int
    items: list[ProviderDetailsResponse]
